  ...
  return (
    <>
      ...
      <div className="mt-4">
        ...
        <select
          className="select select-bordered w-full max-w-xs mt-2 mr-4 dark:bg-transparent dark:text-gray-300"
          onChange={(e) =>
            setSelectedCategory(
              categories.find((item) => item.id === e.target.value) as Tag
            )
          }
        >
          {categories.length > 0 &&
            categories.map((item: Tag, index: number) => (
        ...
        <button
          className="btn btn-sm mt-2 btn-info mr-2 normal-case dark:text-gray-300"
          type="button"
          onClick={() => {
        ...
        <button
          className="btn btn-sm mt-2 btn-outline btn-error normal-case dark:text-gray-300"
          type="button"
          onClick={() => {
            props.clearCategory();
            setCategoryList([]);
      ...
      <div className="flex gap-2 flex-wrap">
        {categoryList.length > 0 &&
          categoryList.map((item: Tag, index: number) => (
