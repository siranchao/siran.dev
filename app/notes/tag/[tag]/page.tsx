...
export default function TagNotes({ params }: { params: { tag: string } }) {
  ...
  const { currentPage, order } = useSelector(
  ...
  return (
    <div className="mb-20">
      <Breadcrumbs
        prevRoute="/"
        currentRoute={`${params.tag.replace("%20", " ")} Notes`}
      />
      <p className="text-2xl mb-4 font-bold">
        Category: {params.tag.replace("%20", " ")}
      </p>
      ...
      <div className="flex justify-start w-full mt-10 mb-8 ">
        <div className="flex items-center">
          ...
          <div>
            <button
              className={`btn btn-sm btn-ghost normal-case rounded-md mx-1 text-gray-600 dark:text-gray-400 ${
                order === "popular" && "btn-active"
              }`}
              onClick={() => selectOrder("popular")}
            >
              Hotest
            </button>
          </div>
